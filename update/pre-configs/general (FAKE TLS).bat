...
set LIST_TITLE=ZAPRET: general_FAKE_TLS
...
--filter-tcp=80 --hostlist="%LIST_PATH%" --dpi-desync=fake,multisplit --dpi-desync-autottl=3 --dpi-desync-fooling=md5sig --new ^
...
--filter-tcp=80 --ipset="%CLOUDFLARE_IPSET_PATH%" --dpi-desync=fake,multisplit --dpi-desync-autottl=3 --dpi-desync-fooling=md5sig --new ^
--filter-tcp=443,%GModeRange% --ipset="%CLOUDFLARE_IPSET_PATH%" --dpi-desync=fake --dpi-desync-ttl=4 --dpi-desync-fake-tls-mod=rnd,rndsni,padencap --new ^
